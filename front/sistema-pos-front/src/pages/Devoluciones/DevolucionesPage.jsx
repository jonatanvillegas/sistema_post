...
import {
  ...
  Select, Tooltip, Descriptions, Divider, Statistic, InputNumber, Form, Radio,
} from 'antd';
import {
...
export default function DevolucionesPage() {
  ...
  // ─── Buscar venta para crear devolución ───
  const handleBuscarVenta = async () => {
    if (!ventaIdBuscar) return;
    try {
      const res = await getProductosVenta(ventaIdBuscar);
  ...
  const handleToggleProducto = (prod, checked) => {
    if (checked) {
      ...
      setProductosSeleccionados((prev) => prev.filter((p) => String(p.productoId) !== String(prod.productoId)));
    }
  };
  ...
  const handleAprobar = async (id) => {
    try {
      ...
      toast.success('Devolución aprobada y stock reingresado');
      fetchData();
      fetchStats();
    } catch (err) {
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          ...
          <Input.TextArea rows={3} placeholder="Motivo del rechazo..."
            onChange={(e) => (motivoRechazo = e.target.value)} />
        </div>
      ),
      okText: 'Rechazar',
      ...
      render: (text, record) => (
        <Space direction="vertical" size={0}>
          <Text strong style={{ cursor: 'pointer', color: '#1677ff' }}
            onClick={() => { setDevolucionDetalle(record); setDetalleVisible(true); }}>
            {text}
          </Text>
            ...
            <>
              <Tooltip title="Aprobar">
                <Button icon={<CheckCircleOutlined />} size="small"
                  ...
                  onClick={() => handleAprobar(record._id)} />
              </Tooltip>
    ...
    <div style={{ padding: '0 24px', width: '100%' }}>
      <div className="page-header" style={{ marginBottom: 24 }}>
        <div>
          ...
          <Text className="page-sub">Gestione las devoluciones de productos y solicitudes de garantía.</Text>
        </div>
        <Button type="primary" size="large" icon={<PlusOutlined />}
          onClick={() => { resetForm(); setCrearVisible(true); }}>
          Nueva Devolución
        </Button>
      </div>
        ...
        <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
          <Col xs={12} sm={6}>
            <Card bordered={false} className="dashboard-card shadow-sm" bodyStyle={{ padding: 16 }}>
              <Statistic title="Este Mes" value={stats.resumenMes?.count || 0} suffix="devoluciones"
                valueStyle={{ fontSize: 22, fontWeight: 700 }} />
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card bordered={false} className="dashboard-card shadow-sm" bodyStyle={{ padding: 16 }}>
              <Statistic title="Total Devuelto" value={stats.resumenMes?.total || 0}
                prefix="C$" precision={2} valueStyle={{ fontSize: 22, fontWeight: 700, color: '#f5222d' }} />
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card bordered={false} className="dashboard-card shadow-sm" bodyStyle={{ padding: 16 }}>
              <Statistic title="Completadas" value={stats.resumenMes?.completadas || 0}
                valueStyle={{ fontSize: 22, fontWeight: 700, color: '#52c41a' }} />
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card bordered={false} className="dashboard-card shadow-sm" bodyStyle={{ padding: 16 }}>
              <Statistic title="Pendientes" value={stats.resumenMes?.pendientes || 0}
      ...
      <Card bordered={false} bodyStyle={{ padding: 0 }} className="dashboard-card shadow-sm">
        <div style={{ padding: '20px 24px' }}>
          <Row gutter={16} align="middle">
            <Col xs={24} md={8}>
              <Input placeholder="Buscar por N° devolución, venta, cliente..."
            ...
            <Col xs={12} md={5}>
              <Select value={tipoFiltro} onChange={(v) => { setTipoFiltro(v); setPage(1); }}
                style={{ width: '100%' }} size="large">
      ...
      <Modal
        ...
        footer={ventaInfo ? [
          ...
          <Button key="create" type="primary" danger loading={creando}
      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          <Text strong style={{ display: 'block', marginBottom: 8 }}>Buscar Venta por ID</Text>
          <Space.Compact style={{ width: '100%' }}>
            <Input placeholder="Ingrese el ID de la venta..." value={ventaIdBuscar}
        ...
        {ventaInfo && (
          <>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <Input.TextArea rows={2} value={motivoGeneral} onChange={(e) => setMotivoGeneral(e.target.value)}
                placeholder="Describa el motivo general de la devolución..." />
            </div>
              ...
              columns={[
                ...
                { title: 'Disp.', dataIndex: 'cantidadDisponible', width: 60, align: 'center' },
                {
                  ...
                  render: (_, record) => {
                    ...
                    return sel ? (
                      <InputNumber min={1} max={record.cantidadDisponible} value={sel.cantidad}
                        size="small" style={{ width: 80 }}
                        onChange={(v) => handleCambiarCantidadDev(record.productoId, v)} />
                    ) : '—';
                  },
                },
                {
                  ...
                  render: (_, record) => {
                    const sel = productosSeleccionados.find((p) => String(p.productoId) === String(record.productoId));
                    return sel ? (
                      <Select size="small" value={sel.motivo} style={{ width: 140 }}
                        onChange={(v) => handleCambiarMotivo(record.productoId, v)}>
        ...
        footer={[
          ...
          devolucionDetalle?.estado === 'pendiente' && isAdmin() && (
            <Button key="approve" type="primary" icon={<CheckCircleOutlined />}
              onClick={() => { setDetalleVisible(false); handleAprobar(devolucionDetalle._id); }}>
              Aprobar
            </Button>
          ),
        ]}
        ...
        {devolucionDetalle && (
          <div>
            <Descriptions bordered size="small" column={{ xs: 1, sm: 2 }}>
              ...
              <Descriptions.Item label="Venta Original">
                <Tag>{devolucionDetalle.numeroVenta}</Tag>
              </Descriptions.Item>
